...
public class GradeDAO {
     ...
     * 从CSV文件导入成绩
     */
    public int importGradesFromCSV(String filePath) throws SQLException {
        List<Grade> grades = new ArrayList<>();
        List<String> errors = new ArrayList<>();

        try {
            grades = parseCSVFile(new File(filePath), errors);
            return batchInsertGrades(grades, errors);
        } catch (Exception e) {
            throw new SQLException("CSV文件导入失败: " + e.getMessage());
        }
    }
     ...
     * 从Excel文件导入成绩
     */
    public int importGradesFromExcel(String filePath) throws SQLException {
        List<Grade> grades = new ArrayList<>();
        List<String> errors = new ArrayList<>();

        try {
            grades = parseExcelFile(new File(filePath), errors);
            return batchInsertGrades(grades, errors);
        } catch (Exception e) {
            throw new SQLException("Excel文件导入失败: " + e.getMessage());
        }
    }
     ...
     * 批量插入成绩数据
     */
    private int batchInsertGrades(List<Grade> grades, List<String> errors) throws SQLException {
        if (grades.isEmpty()) {
            return 0;
        }
            ...
            for (Grade grade : grades) {
                try {
                    ...
                    if (!studentExists(grade.getStudentId())) {
                        errors.add("学生不存在: " + grade.getStudentId());
                        continue;
                    }
                    ...
                    errors.add("处理记录失败 " + grade.getStudentId() + ": " + e.getMessage());
                }
            }
